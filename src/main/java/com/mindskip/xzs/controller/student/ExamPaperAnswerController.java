...
import com.mindskip.xzs.viewmodel.admin.userCredential.userCredentialCreateVM;
...
public class ExamPaperAnswerController extends BaseApiController {
    ...
    @Autowired
    public ExamPaperAnswerController(ExamPaperAnswerService examPaperAnswerService, ExamPaperService examPaperService,
        SubjectService subjectService,CredentialService credentialService , UserCredentialService userCredentialService,
    ...
    public RestResponse answerSubmit(@RequestBody @Valid ExamPaperSubmitVM examPaperSubmitVM) {
        ...
        Credential credential = credentialService.getCredentialById(examPaperAnswerInfo.getExamPaper().getId());
        if(credential.getWeight()==null){
            key =0.6;
        }
        else {
            key= credential.getWeight();
        }

        Double standardValue = examPaperAnswerInfo.getExamPaper().getScore()*key;
        if(userScore>=standardValue) {
            userCredential.setExamPaperName(examPaperAnswerInfo.getExamPaper().getName());
            userCredential.setCreateTime(now);
            userCredential.setUserId(user.getId());
            userCredential.setUrl(credential.getUrl());
            userCredentialService.insertByFilter(userCredential);
        }
